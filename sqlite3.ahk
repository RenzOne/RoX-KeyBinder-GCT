      ...
      If (This.Base._RefCount = 0) {
         ...
         If !FileExist(SQLiteDLL)
            If FileExist(A_ScriptDir . "\SQLiteDB.ini") {
               IniRead(A_ScriptDir "\SQLiteDB.ini", "Main", "DllPath", SQLiteDLL)
         ...
         If (SQLVersion[1] < MinVersion[1]) || ((SQLVersion[1] = MinVersion[1]) && (SQLVersion[2] < MinVersion[2])){
            ...
            MsgBox("Version " . This.Base.Version .  " of SQLite3.dll is not supported!`n`n"
                                      . "You can download the current version from www.sqlite.org!",SQLiteDB Error,16)
            ExitApp
         }
      }
...
Exec(SQL, Callback := "") {
	  ...
	  RC := DllCall("SQlite3.dll\sqlite3_exec", "Ptr", This._Handle, "Ptr", &UTF8, "Int", CBPtr, "Ptr", "", "PtrP", Err, "Cdecl Int")
